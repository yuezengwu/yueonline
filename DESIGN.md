...
> 首页固定文案以 `CONTENT.md` 为准（YUE 随时改）；落地到 `lib/site.ts` 后页面才会更新。Writing 条目例外：直接读取最近 2 篇已发布文章的 frontmatter。
...
- 首页 Writing:从 `content/blog/*.md` 自动读取最近 2 篇 `draft: false` 的文章，不在主页配置中重复维护
